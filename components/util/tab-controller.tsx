...
function TabController({
	...
	const slideTo = () => {
		setWidth(
			(refs.current[position.current]! as Element).getBoundingClientRect().width
		);
		offsetLeft.current = (
			offsetRef.current! as Element
		).getBoundingClientRect().left;
		setLm(
			(refs.current[position.current]! as Element).getBoundingClientRect()
	...
	return (
		<div className="w-full overflow-x-scroll">
			...
			<div
				...
				className={`h-[2.5px] overflow-hidden relative w-full flex items-center rounded-full`}
			>
				{canShowIndicator ? (
					<div
						style={{
